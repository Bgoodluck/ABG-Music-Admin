...
function AddMusic() {
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/song/add`, formData);
    ...
    <form onSubmit={onSubmitHandler} className=' flex flex-col items-start gap-8 text-gray-600'>
      <div className=' flex gap-8'>
        <div className=' flex flex-col gap-4'>
            ...
            <label htmlFor='song'>
                <img src={song ? assets.upload_added : assets.upload} className=' w-24 cursor-pointer' alt="" />
            </label>
        </div>
        <div className=' flex flex-col gap-4'>
            <p>Upload Image</p>
            ...
            <label htmlFor="image">
                <img src={image ? URL.createObjectURL(image) : assets.upload} className=' w-24 cursor-pointer' alt="" />
            </label>
        </div>
      </div>
      ...
      <div className=' flex flex-col gap-2.5'>
        <p>Artist</p>
        <input onChange={(e) => setArtist(e.target.value)} value={artist} className=' bg-transparent outline-green-600 border-2 border-gray-400 p-2.5 w-[max(40vw, 250px)]' placeholder='Type Here' type='text' required />
      </div>
